...
class BkUser(AbstractBaseUser, PermissionsMixin):
    ...
    @staticmethod
    def get_chname_by_username(username_list):
        user_list = BkUser.objects.filter(username__in=username_list).values('username', 'chname')
        return {user['username']: user['chname'] for user in user_list}
