...
function transform_solution_data(solution_data, solution_type){
    ...
    for(var i in solution_data){
        nodes.push({
            "id": solution_data[i][4],
            "text": solution_data[i][2],
            "solution_id": solution_data[i][1],
            className: solution_data[i][3],
            height: '60',
            width: '130'})
        ...
        if(relation != {}){
            for(var re in relation){
                edges.push({
                    "source": solution_data[i][4],
                    "target": solution_data[re][4],
                    edgesType: solution_type[relation[re][0]]
                })
            }
        }
    }
